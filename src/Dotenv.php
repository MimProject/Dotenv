...
namespace Mim\Component\Dotenv;


use Mim\Component\Dotenv\Exceptions\FormatException;
use Mim\Component\Dotenv\Exceptions\FormatExceptionContext;
 ...
 * Manages .env files.
 *
 * @author Théo Sikli <theo@theo-sikli.fr>
 */
final class Dotenv
{
    /**
    ...
    public function populate(array $values, bool $overrideExistingVars = false): void
    {
        ...
        $loadedVars = array_flip(explode(',', $_SERVER['MIM_DOTENV_VARS'] ?? $_ENV['MIM_DOTENV_VARS'] ?? ''));
        ...
        if ($updateLoadedVars) {
            ...
            $_ENV['MIM_DOTENV_VARS'] = $_SERVER['MIM_DOTENV_VARS'] = $loadedVars;

            if ($this->usePutenv) {
                putenv('MIM_DOTENV_VARS=' . $loadedVars);
            }
        }
    }
